...
{
    if (max === undefined) {
        max = pixelData[pixelData.length - 1];
    }

    for (let i = 0; i < pixelData.length; i++) {
        const value = Math.floor(pixelData[i] / max * 0xFFFFFF);
...
export class HaarClassifierShader extends Filter
{
    ...
    public detectObject(image: Image): Image
    {
        ...
        const iiMax = convertImageF32I888A(this.integralImage.ii, this.iiBuffer.getImageData().data);
        const ii2Max = convertImageF32I888A(this.integralImage.ii2, this.ii2Buffer.getImageData().data);

        const iiTexture = this.iiBuffer.asWebGLTexture();
        const ii2Texture = this.ii2Buffer.asWebGLTexture();
